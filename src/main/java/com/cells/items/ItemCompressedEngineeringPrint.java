...
/**
 * Compressed Engineering Processor Prints.
 *
 ...
 */
public class ItemCompressedEngineeringPrint extends Item {
    ...
    public ItemCompressedEngineeringPrint() {
        setMaxStackSize(64);
        setHasSubtypes(true);
        setMaxDamage(0);
        setCreativeTab(CellsCreativeTab.instance);
        setRegistryName(Tags.MODID, "compressed_engineering_print");
        setTranslationKey(Tags.MODID + ".compressed_engineering_print");
    }
    ...
    /**
     * Create a compressed engineering print ItemStack.
    ...
    public static ItemStack create(int level) {
        if (ItemRegistry.COMPRESSED_ENGINEERING_PRINT == null) return ItemStack.EMPTY;
        if (level < 0 || level >= LEVEL_NAMES.length) level = 0;

        return new ItemStack(ItemRegistry.COMPRESSED_ENGINEERING_PRINT, 1, level);
    }
