...
/**
 * Compressed Calculation Processor Prints.
 * <p>
 ...
 */
public class ItemCompressedCalculationPrint extends Item {
    ...
    public ItemCompressedCalculationPrint() {
        setMaxStackSize(64);
        setHasSubtypes(true);
        setMaxDamage(0);
        setCreativeTab(CellsCreativeTab.instance);
        setRegistryName(Tags.MODID, "compressed_calculation_print");
        setTranslationKey(Tags.MODID + ".compressed_calculation_print");
    }
    ...
    /**
     * Create a compressed calculation print ItemStack.
    ...
    public static ItemStack create(int level) {
        if (ItemRegistry.COMPRESSED_CALCULATION_PRINT == null) return ItemStack.EMPTY;
        if (level < 0 || level >= LEVEL_NAMES.length) level = 0;

        return new ItemStack(ItemRegistry.COMPRESSED_CALCULATION_PRINT, 1, level);
    }
